...
#ifndef MUTIL_IOBUF_H
#define MUTIL_IOBUF_H
...
namespace mutil {
    ...
    public:
        ...
        // Explicitly declare this overload as error to avoid copy_to(mutil::IOBuf*)
    ...
    class IOBufCutter {
    public:
        explicit IOBufCutter(mutil::IOBuf *buf);

        ~IOBufCutter();

        // Cut off n bytes and APPEND to `out'
        // Returns bytes cut.
        size_t cutn(mutil::IOBuf *out, size_t n);
    ...
    private:
        const mutil::IOBuf *_buf;
        mutil::IOBufAsZeroCopyInputStream _stream;
    };
...
// Wrap IOBuf into output of snappy compression.
    ...
    private:
        ...
        int _cur_len;
        mutil::IOBuf *_buf;
        mutil::IOBufAsZeroCopyOutputStream _buf_stream;
    };
...
// During iteration, the iobuf should NOT be changed.
    ...
    public:
        explicit IOBufBytesIterator(const mutil::IOBuf &buf);
        ...
        size_t append_and_forward(mutil::IOBuf *buf, size_t n);
...
namespace std {
    template<>
    inline void swap(mutil::IOBuf &a, mutil::IOBuf &b) {
        return a.swap(b);
    }
} // namespace std
