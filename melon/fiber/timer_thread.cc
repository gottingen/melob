...
namespace fiber {
    ...
    inline TimerThread::TaskId make_task_id(
            mutil::ResourceId<TimerThread::Task> slot, uint32_t version) {
        return TimerThread::TaskId((((uint64_t) version) << 32) | slot.value);
    }

    inline
    mutil::ResourceId<TimerThread::Task> slot_of_task_id(TimerThread::TaskId id) {
        mutil::ResourceId<TimerThread::Task> slot = {(id & 0xFFFFFFFFul)};
        return slot;
    }
    ...
    TimerThread::TaskId TimerThread::schedule(
        ...
        const Bucket::ScheduleResult result =
                _buckets[mutil::fmix64(pthread_numeric_id()) % _options.num_buckets]
                        .schedule(fn, arg, abstime);
        if (result.earlier) {
            bool earlier = false;
            const int64_t run_time = mutil::timespec_to_microseconds(abstime);
            {
                MELON_SCOPED_LOCK(_mutex);
    ...
    void TimerThread::run() {
        ...
        while (!_stop.load(std::memory_order_relaxed)) {
            ...
            while (!tasks.empty()) {
                ...
                if (mutil::gettimeofday_us() < task1->run_time) {  // not ready yet.
                    break;
                }
            ...
            if (next_run_time != std::numeric_limits<int64_t>::max()) {
                next_timeout = mutil::microseconds_to_timespec(next_run_time - now);
