...
struct SampledContention : public melon::var::Collected {
    ...
    size_t hash_code() const {
        ...
        if (_hash_code == 0) {
            ...
            mutil::MurmurHash3_x86_32(stack, sizeof(void*) * nframes, seed, &_hash_code);
        }
        return _hash_code;
    }
private:
friend mutil::ObjectPool<SampledContention>;
    SampledContention()
...
public:
    typedef mutil::FlatMap<SampledContention*, SampledContention*,
...
private:
    ...
    mutil::IOBuf _disk_buf;  // temp buf before saving the file.
...
MELON_CACHELINE_ALIGNMENT static ContentionProfiler* g_cp = NULL;
...
//   #1  0x00000000004b4df7 in mutil::internal::SpinLockDelay(int volatile*, int, int) ()
...
//   #9  0x0000000000666517 in fiber::init_sys_mutex_lock () at fiber/mutex.cpp:358
//   #10 0x00007effddc97a90 in pthread_once () at ../nptl/sysdeps/unix/sysv/linux/x86_64/pthread_once.S:103
//   #11 0x000000000066649f in fiber::first_sys_pthread_mutex_lock (mutex=0xbaf880 <_ULx86_64_lock>) at fiber/mutex.cpp:366
//   #12 0x00000000006678bc in pthread_mutex_lock_impl (mutex=0xbaf880 <_ULx86_64_lock>) at fiber/mutex.cpp:489
//   #13 pthread_mutex_lock (__mutex=__mutex@entry=0xbaf880 <_ULx86_64_lock>) at fiber/mutex.cpp:751
...
inline uint64_t hash_mutex_ptr(const pthread_mutex_t* m) {
    return mutil::fmix64((uint64_t)m);
}
...
void submit_contention(const fiber_contention_site_t& csite, int64_t now_ns) {
    ...
    auto sc = mutil::get_object<SampledContention>();
...
MUTIL_FORCE_INLINE int pthread_mutex_lock_impl(pthread_mutex_t* mutex) {
    ...
    if (!rc) { // Inside lock
        ...
        csite->duration_ns = mutil::cpuwide_time_ns() - start_ns;
        csite->sampling_range = sampling_range;
    } // else rare
    return rc;
}

MUTIL_FORCE_INLINE int pthread_mutex_unlock_impl(pthread_mutex_t* mutex) {
    ...
    for (int i = fast_alt.count - 1; i >= 0; --i) {
        if (fast_alt.list[i].mutex == mutex) {
            if (is_contention_site_valid(fast_alt.list[i].csite)) {
                ...
                unlock_start_ns = mutil::cpuwide_time_ns();
            }
    ...
    // [Outside lock]
    if (unlock_start_ns) {
        const int64_t unlock_end_ns = mutil::cpuwide_time_ns();
...
int fiber_mutex_init(fiber_mutex_t* __restrict m,
    ...
    m->butex = fiber::butex_create_checked<unsigned>();
    if (!m->butex) {
        return ENOMEM;
    }
    *m->butex = 0;
...
int fiber_mutex_lock_contended(fiber_mutex_t* m) {
    return fiber::mutex_lock_contended(m);
}
...
int fiber_mutex_lock(fiber_mutex_t* m) {
    ...
    const size_t sampling_range = melon::var::is_collectable(&fiber::g_cp_sl);
    if (!sampling_range) { // Don't sample
        return fiber::mutex_lock_contended(m);
    }
    // Start sampling.
    ...
    const int rc = fiber::mutex_lock_contended(m);
    if (!rc) { // Inside lock
        m->csite.duration_ns = mutil::cpuwide_time_ns() - start_ns;
...
int fiber_mutex_timedlock(fiber_mutex_t* __restrict m,
    ...
    // Don't sample when contention profiler is off.
    if (!fiber::g_cp) {
        return fiber::mutex_timedlock_contended(m, abstime);
    }
    // Ask Collector if this (contended) locking should be sampled.
    const size_t sampling_range = melon::var::is_collectable(&fiber::g_cp_sl);
    if (!sampling_range) { // Don't sample
    ...
    // NOTE: Don't modify m->csite outside lock since multiple threads are
    // still contending with each other.
    const int rc = fiber::mutex_timedlock_contended(m, abstime);
    if (!rc) { // Inside lock
        m->csite.duration_ns = mutil::cpuwide_time_ns() - start_ns;
        m->csite.sampling_range = sampling_range;
    } else if (rc == ETIMEDOUT) {
        // Failed to lock due to ETIMEDOUT, submit the elapse directly.
        const int64_t end_ns = mutil::cpuwide_time_ns();
        const fiber_contention_site_t csite = {end_ns - start_ns, sampling_range};
        fiber::submit_contention(csite, end_ns);
    }
    return rc;
}
...
int fiber_mutex_unlock(fiber_mutex_t* m) {
    ...
    // Wakeup one waiter
    if (!fiber::is_contention_site_valid(saved_csite)) {
        fiber::butex_wake(whole);
        return 0;
    }
    const int64_t unlock_start_ns = mutil::cpuwide_time_ns();
    fiber::butex_wake(whole);
    const int64_t unlock_end_ns = mutil::cpuwide_time_ns();
    saved_csite.duration_ns += unlock_end_ns - unlock_start_ns;
    fiber::submit_contention(saved_csite, unlock_end_ns);
    return 0;
}

int pthread_mutex_lock (pthread_mutex_t *__mutex) {
    return fiber::pthread_mutex_lock_impl(__mutex);
}
int pthread_mutex_unlock (pthread_mutex_t *__mutex) {
    return fiber::pthread_mutex_unlock_impl(__mutex);
}
